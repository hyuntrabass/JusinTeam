...
#pragma region 이펙트
		...
		{
			/* Effect */
			m_eItemType = ItemType::Effect;

			_int m_iEffectCount = m_EffectDummyList.size();

			ImGui::InputInt("Effects Count", &m_iEffectCount, 14);
			ImGui::Separator();
			ImGui::InputFloat("Size", &m_fEffectSize, 1.f, 0.1f, "%.3f", 0);
			ImGui::Separator();
			ImGui::InputFloat4("Position", &m_EffectMatrix.m[3][0], 0);
			ImGui::Separator();
			static int Effect_current_idx = 0;
			...
			for (int i = 0; i < m_EffectFiles.size(); ++i)
			{
				if (Filter.PassFilter(m_EffectFiles[i].c_str()))
				{
					_bool isSelected = (i == m_iSelectIdx);

					if (ImGui::Selectable(m_EffectFiles[i].c_str(), isSelected))
					{
						Effect_current_idx = i;
						m_iSelectIdx = Effect_current_idx;
					}
					if (isSelected)
					{
						ImGui::SetItemDefaultFocus();
			...
			ImGui::Separator();
			ImGui::SeparatorText("MATRIX : ");
			ImGui::InputFloat4("Right", &m_EffectMatrix.m[0][0], 0);
			ImGui::InputFloat4("Up", &m_EffectMatrix.m[1][0], 0);
