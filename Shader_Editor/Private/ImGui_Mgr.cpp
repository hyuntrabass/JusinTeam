...
void CImGui_Mgr::Frame()
{
	...
	ImGui::Begin(u8"»ÇÅ¥");
	if (ImGui::BeginTabBar(u8"½¦ÀÌ´õ")) {
		if (ImGui::BeginTabItem(u8"¾Æ½êÀÌ AO")) {
			ImGui::DragFloat(u8"Intensity(°­µµ)", &m_SSAO.fIntensity, 0.5f, 0.001f, 100.f);
			ImGui::DragFloat(u8"Radius(¹ÝÁö¸§)", &m_SSAO.fRadius, 0.001f, 0.001f, 5.f);
			ImGui::DragFloat(u8"Scale(»çÀÌ °Å¸®)", &m_SSAO.fScale, 0.01f, 0.001f, 100.f);
			ImGui::DragFloat(u8"Bias(Â÷Æó¹°ÀÇ ³Êºñ)", &m_SSAO.fBias, 0.01f, 0.001f, 100.f);
