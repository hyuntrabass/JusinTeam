...
CVehicleDesc::CVehicleDesc(_dev pDevice, _context pContext)
	: COrthographicObject(pDevice, pContext)
{
}

CVehicleDesc::CVehicleDesc(const CVehicleDesc& rhs)
	: COrthographicObject(rhs)
{
}

HRESULT CVehicleDesc::Init_Prototype()
{
	m_isPrototype = true;
	return S_OK;
}

HRESULT CVehicleDesc::Init(void* pArg)
{
...
void CVehicleDesc::Tick(_float fTimeDelta)
{
	m_rcRect = {
	  (LONG)(m_fX - m_fSizeX * 0.5f),
	  (LONG)(m_fY - m_fSizeY * 0.5f),
...
void CVehicleDesc::Late_Tick(_float fTimeDelta)
{
	if (m_bSelect)
	{
...
const SKILLINFO& CVehicleDesc::Get_SkillInfo() const
{
	return m_tSkillInfo;
}
void CVehicleDesc::Set_Position(_vec2 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;	
	__super::Apply_Orthographic(g_iWinSizeX, g_iWinSizeY);
...
void CVehicleDesc::Unlock_Skill()
{
	if (m_eCurType == LOCK)
	{
		m_eCurType = UNLOCK;
...
void CVehicleDesc::Select_Skill(_bool isSelect)
{
...
HRESULT CVehicleDesc::Add_Parts()
{
...
HRESULT CVehicleDesc::Add_Components()
{
...
HRESULT CVehicleDesc::Bind_ShaderResources()
{
...
CVehicleDesc* CVehicleDesc::Create(_dev pDevice, _context pContext)
{
	CVehicleDesc* pInstance = new CVehicleDesc(pDevice, pContext);
...
CGameObject* CVehicleDesc::Clone(void* pArg)
{
	CVehicleDesc* pInstance = new CVehicleDesc(*this);

	if (FAILED(pInstance->Init(pArg)))
	{
		MSG_BOX("Failed to Clone : CVehicleDesc");
		Safe_Release(pInstance);
	}
